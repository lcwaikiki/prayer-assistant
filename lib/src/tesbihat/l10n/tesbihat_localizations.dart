import 'package:flutter/material.dart';

class TesbihatLocalizations {
  TesbihatLocalizations(this.locale);

  final Locale locale;

  static const supportedLocales = [
    Locale('en'),
    Locale('es'),
    Locale('fr'),
    Locale('de'),
    Locale('fa'),
    Locale('ar'),
    Locale('ja'),
    Locale('zh'),
    Locale('tr'),
    Locale('ur'),
    Locale('ru'),
    Locale('bn'),
    Locale('ta'),
  ];

  static const localizationsDelegates = [_TesbihatLocalizationsDelegate()];

  static TesbihatLocalizations of(BuildContext context) {
    final localizations = Localizations.of<TesbihatLocalizations>(
      context,
      TesbihatLocalizations,
    );
    return localizations ?? TesbihatLocalizations(const Locale('en'));
  }

  static const _strings = <String, Map<String, String>>{
    'en': {
      'appTitle': 'Beads Counter',
      'milestones': 'Beads',
      'switchToLight': 'Switch to light mode',
      'switchToDark': 'Switch to dark mode',
      'language': 'Language',
      'chooseLanguage': 'Choose language',
      'noMilestones': 'No beads yet. Tap + to add one.',
      'statsTitle': 'History',
      'statsToday': 'Today',
      'statsLast7Days': '7 days',
      'statsTotal': 'Total',
      'deleted': 'deleted',
      'undo': 'Undo',
      'edit': 'Edit',
      'duplicate': 'Duplicate',
      'clear': 'Clear',
      'delete': 'Delete',
      'count': 'Count',
      'check': 'Check',
      'set': 'Set',
      'progress': 'Progress',
      'createMilestone': 'Create Beads',
      'editMilestone': 'Edit Beads',
      'title': 'Title',
      'notes': 'Notes',
      'notesHint': 'Add notes for these beads...',
      'countField': 'Count',
      'checkInterval': 'Check Interval',
      'checkHelper': 'Less or equal to half of count. Leave empty or 0 for no checkpoints.',
      'setCount': 'Set Count',
      'setCountHelper': 'Set count must be less than or equal to count.',
      'setCountReadonlyHelper':
          'Set count can only be changed from progress screen.',
      'vibrationIntensity': 'Vibration Intensity',
      'reminderTitle': 'Reminder',
      'reminderEnable': 'Enable reminder',
      'reminderRepeatOnce': 'Once',
      'reminderRepeatDaily': 'Daily',
      'reminderRepeatWeekly': 'Weekly',
      'reminderRepeatMonthly': 'Monthly',
      'reminderRepeatYearly': 'Yearly',
      'reminderRepeatCountLabel': 'Repeat count',
      'reminderRepeatCountHelper':
          'Number of times the reminder fires before stopping (off = repeats forever)',
      'reminderRepeatCountRangeError': 'Enter a number from 2 to 100',
      'reminderRepeatDaysLabel': 'Repeat on',
      'reminderDayOfMonthLabel': 'Day of month',
      'reminderYearlyMonthLabel': 'Month',
      'reminderYearlyDayLabel': 'Day',
      'reminderRecurrenceLabel': 'Repeat',
      'reminderMonthlyBasisLabel': 'Monthly basis',
      'reminderYearlyBasisLabel': 'Yearly basis',
      'reminderBasisGregorian': 'Gregorian',
      'reminderBasisHijri': 'Hijri',
      'reminderPickDateTime': 'Pick date & time',
      'reminderPickDate': 'Pick date',
      'reminderPickTime': 'Pick time',
      'reminderNotSet': 'Not set',
      'reminderAnchorTime': 'Time',
      'reminderAnchorPrayer': 'Prayer time',
      'reminderOffsetOnTime': 'On time',
      'reminderOffsetBefore': 'Before',
      'reminderOffsetAfter': 'After',
      'reminderMinutesLabel': 'Minutes',
      'reminderSelectPrayer': 'Select prayer',
      'save': 'Save',
      'update': 'Update',
      'requiredSuffix': 'is required',
      'mustBeInteger': 'must be an integer',
      'countPositive': 'Count must be a positive integer',
      'checkGreaterThanZero': 'Check must be greater than 0',
      'checkHalfError': 'Check cannot be greater than half of count',
      'enterValidCountFirst': 'Enter a valid count first',
      'setCountNegative': 'Set count cannot be negative',
      'setCountGreaterCount': 'Set count cannot be greater than count',
      'setCountValueRequired': 'Set count is required',
      'itemNotFound': 'Item not found',
      'resetProgressTitle': 'Reset progress?',
      'resetProgressBody': 'This will set current progress back to 0.',
      'cancel': 'Cancel',
      'reset': 'Reset',
      'editProgressAndSetCount': 'Edit progress and set count',
      'progressCount': 'Progress Count',
      'setCountCannotNegative': 'Set count cannot be negative',
      'validProgressNumber': 'Enter a valid progress number',
      'progressBetween': 'Progress must be between 0 and {max}',
      'maxMinusCount': 'Left Count',
      'tap': 'TAP',
      'groups': 'Groups',
      'newGroup': 'New Group',
      'editGroup': 'Edit Group',
      'groupName': 'Group name',
      'groupMembers': 'Members',
      'noBeadsInGroup': 'No beads in this group yet.',
      'addBead': 'Add bead',
      'addBeads': 'Add beads',
      'newBead': 'New Bead',
      'deleteGroup': 'Delete Group',
      'deleteGroupConfirm': 'Delete this group? Its beads are kept.',
      'removeFromGroup': 'Remove from group',
      'noNotesAdded': 'No notes added.',
      'select': 'Select',
      'selectAll': 'Select all',
      'selectedCount': '{count} selected',
      'deleteSelectedConfirm': 'Delete {count} selected items?',
      'deletedSelected': '{count} deleted',
    },
    'es': {
      'appTitle': 'Contador de Cuentas',
      'milestones': 'Cuentas',
      'switchToLight': 'Cambiar a modo claro',
      'switchToDark': 'Cambiar a modo oscuro',
      'language': 'Idioma',
      'chooseLanguage': 'Elegir idioma',
      'noMilestones': 'Aun no hay cuentas. Toca + para agregar una.',
      'statsTitle': 'Historial',
      'statsToday': 'Hoy',
      'statsLast7Days': '7 días',
      'statsTotal': 'Total',
      'deleted': 'eliminado',
      'undo': 'Deshacer',
      'edit': 'Editar',
      'duplicate': 'Duplicar',
      'clear': 'Borrar',
      'delete': 'Eliminar',
      'count': 'Conteo',
      'check': 'Chequeo',
      'set': 'Serie',
      'progress': 'Progreso',
      'createMilestone': 'Crear Cuentas',
      'editMilestone': 'Editar Cuentas',
      'title': 'Titulo',
      'notes': 'Notas',
      'notesHint': 'Agrega notas para estas cuentas...',
      'countField': 'Conteo',
      'checkInterval': 'Intervalo de chequeo',
      'checkHelper': 'Debe ser menor o igual a la mitad del conteo. Usa 0 para sin puntos de control.',
      'setCount': 'Conteo de series',
      'setCountHelper': 'El conteo de series debe ser menor o igual al conteo.',
      'setCountReadonlyHelper':
          'El conteo de series solo se puede cambiar desde la pantalla de progreso.',
      'vibrationIntensity': 'Intensidad de vibracion',
      'reminderTitle': 'Recordatorio',
      'reminderEnable': 'Activar recordatorio',
      'reminderRepeatOnce': 'Una vez',
      'reminderRepeatDaily': 'Diario',
      'reminderRepeatWeekly': 'Semanal',
      'reminderRepeatMonthly': 'Mensual',
      'reminderRepeatYearly': 'Anual',
      'reminderRepeatCountLabel': 'Número de repeticiones',
      'reminderRepeatCountHelper':
          'Cuántas veces se activa el recordatorio antes de detenerse (desactivado = se repite siempre)',
      'reminderRepeatCountRangeError': 'Introduce un número del 2 al 100',
      'reminderRepeatDaysLabel': 'Repetir en',
      'reminderDayOfMonthLabel': 'Día del mes',
      'reminderYearlyMonthLabel': 'Mes',
      'reminderYearlyDayLabel': 'Día',
      'reminderRecurrenceLabel': 'Repetición',
      'reminderMonthlyBasisLabel': 'Base mensual',
      'reminderYearlyBasisLabel': 'Base anual',
      'reminderBasisGregorian': 'Gregoriano',
      'reminderBasisHijri': 'Hégira',
      'reminderPickDateTime': 'Elegir fecha y hora',
      'reminderPickDate': 'Elegir fecha',
      'reminderPickTime': 'Elegir hora',
      'reminderNotSet': 'No establecido',
      'reminderAnchorTime': 'Hora',
      'reminderAnchorPrayer': 'Hora de oración',
      'reminderOffsetOnTime': 'A la hora',
      'reminderOffsetBefore': 'Antes',
      'reminderOffsetAfter': 'Después',
      'reminderMinutesLabel': 'Minutos',
      'reminderSelectPrayer': 'Seleccionar oración',
      'save': 'Guardar',
      'update': 'Actualizar',
      'requiredSuffix': 'es obligatorio',
      'mustBeInteger': 'debe ser un numero entero',
      'countPositive': 'El conteo debe ser un entero positivo',
      'checkGreaterThanZero': 'El chequeo debe ser mayor que 0',
      'checkHalfError': 'El chequeo no puede ser mayor que la mitad del conteo',
      'enterValidCountFirst': 'Ingresa primero un conteo valido',
      'setCountNegative': 'El conteo de series no puede ser negativo',
      'setCountGreaterCount':
          'El conteo de series no puede ser mayor que el conteo',
      'setCountValueRequired': 'El conteo de series es obligatorio',
      'itemNotFound': 'Elemento no encontrado',
      'resetProgressTitle': 'Reiniciar progreso?',
      'resetProgressBody': 'Esto pondra el progreso actual en 0.',
      'cancel': 'Cancelar',
      'reset': 'Reiniciar',
      'editProgressAndSetCount': 'Editar progreso y conteo de series',
      'progressCount': 'Conteo de progreso',
      'setCountCannotNegative': 'El conteo de series no puede ser negativo',
      'validProgressNumber': 'Ingresa un numero de progreso valido',
      'progressBetween': 'El progreso debe estar entre 0 y {max}',
      'maxMinusCount': 'Conteo restante',
      'tap': 'TOCAR',
      'groups': 'Grupos',
      'newGroup': 'Nuevo grupo',
      'editGroup': 'Editar grupo',
      'groupName': 'Nombre del grupo',
      'groupMembers': 'Miembros',
      'noBeadsInGroup': 'No hay cuentas en este grupo todavía.',
      'addBead': 'Añadir cuenta',
      'addBeads': 'Añadir cuentas',
      'newBead': 'Nueva cuenta',
      'deleteGroup': 'Eliminar grupo',
      'deleteGroupConfirm': '¿Eliminar este grupo? Sus cuentas se conservan.',
      'removeFromGroup': 'Quitar del grupo',
      'noNotesAdded': 'No se agregaron notas.',
      'select': 'Seleccionar',
      'selectAll': 'Seleccionar todo',
      'selectedCount': '{count} seleccionados',
      'deleteSelectedConfirm': '¿Eliminar {count} elementos seleccionados?',
      'deletedSelected': '{count} eliminados',
    },
    'fa': {
      'appTitle': 'شمارنده تسبیح',
      'milestones': 'تسبیح',
      'switchToLight': 'تغییر به حالت روشن',
      'switchToDark': 'تغییر به حالت تیره',
      'language': 'زبان',
      'chooseLanguage': 'انتخاب زبان',
      'noMilestones': 'هنوز تسبیحی وجود ندارد. برای افزودن + را بزنید.',
      'statsTitle': 'تاریخچه',
      'statsToday': 'امروز',
      'statsLast7Days': '۷ روز',
      'statsTotal': 'مجموع',
      'deleted': 'حذف شد',
      'undo': 'بازگردانی',
      'edit': 'ویرایش',
      'delete': 'حذف',
      'count': 'شمارش',
      'check': 'بررسی',
      'set': 'دور',
      'progress': 'پیشرفت',
      'createMilestone': 'ایجاد تسبیح',
      'editMilestone': 'ویرایش تسبیح',
      'title': 'عنوان',
      'notes': 'یادداشت‌ها',
      'notesHint': 'برای این تسبیح یادداشت بنویسید...',
      'countField': 'شمارش',
      'checkInterval': 'فاصله بررسی',
      'checkHelper': 'باید کمتر یا مساوی نصف شمارش باشد. برای بدون نقطه کنترل ۰ وارد کنید.',
      'setCount': 'تعداد دور',
      'setCountHelper': 'تعداد دور باید کمتر یا مساوی شمارش باشد.',
      'setCountReadonlyHelper': 'تعداد دور فقط از صفحه پیشرفت قابل تغییر است.',
      'vibrationIntensity': 'شدت ویبره',
      'reminderTitle': 'یادآور',
      'reminderEnable': 'فعال‌سازی یادآور',
      'reminderRepeatOnce': 'یک‌بار',
      'reminderRepeatDaily': 'روزانه',
      'reminderRepeatWeekly': 'هفتگی',
      'reminderRepeatMonthly': 'ماهانه',
      'reminderRepeatYearly': 'سالانه',
      'reminderRepeatCountLabel': 'تعداد تکرار',
      'reminderRepeatCountHelper':
          'تعداد دفعاتی که یادآوری قبل از توقف اجرا می‌شود (خاموش = همیشه تکرار می‌شود)',
      'reminderRepeatCountRangeError': 'عددی از ۲ تا ۱۰۰ وارد کنید',
      'reminderRepeatDaysLabel': 'تکرار در',
      'reminderDayOfMonthLabel': 'روز ماه',
      'reminderYearlyMonthLabel': 'ماه',
      'reminderYearlyDayLabel': 'روز',
      'reminderRecurrenceLabel': 'تکرار',
      'reminderMonthlyBasisLabel': 'مبنای ماهانه',
      'reminderYearlyBasisLabel': 'مبنای سالانه',
      'reminderBasisGregorian': 'میلادی',
      'reminderBasisHijri': 'هجری',
      'reminderPickDateTime': 'انتخاب تاریخ و ساعت',
      'reminderPickDate': 'انتخاب تاریخ',
      'reminderPickTime': 'انتخاب ساعت',
      'reminderNotSet': 'تنظیم نشده',
      'reminderAnchorTime': 'ساعت',
      'reminderAnchorPrayer': 'وقت نماز',
      'reminderOffsetOnTime': 'در زمان',
      'reminderOffsetBefore': 'قبل',
      'reminderOffsetAfter': 'بعد',
      'reminderMinutesLabel': 'دقیقه',
      'reminderSelectPrayer': 'انتخاب نماز',
      'save': 'ذخیره',
      'update': 'به‌روزرسانی',
      'requiredSuffix': 'الزامی است',
      'mustBeInteger': 'باید عدد صحیح باشد',
      'countPositive': 'شمارش باید عدد صحیح مثبت باشد',
      'checkGreaterThanZero': 'مقدار بررسی باید بیشتر از 0 باشد',
      'checkHalfError': 'مقدار بررسی نمی‌تواند بیشتر از نصف شمارش باشد',
      'enterValidCountFirst': 'ابتدا یک شمارش معتبر وارد کنید',
      'setCountNegative': 'تعداد دور نمی‌تواند منفی باشد',
      'setCountGreaterCount': 'تعداد دور نمی‌تواند بیشتر از شمارش باشد',
      'setCountValueRequired': 'تعداد دور الزامی است',
      'itemNotFound': 'مورد پیدا نشد',
      'resetProgressTitle': 'پیشرفت بازنشانی شود؟',
      'resetProgressBody': 'این کار پیشرفت فعلی را به 0 برمی‌گرداند.',
      'cancel': 'لغو',
      'reset': 'بازنشانی',
      'editProgressAndSetCount': 'ویرایش پیشرفت و تعداد دور',
      'progressCount': 'شمارش پیشرفت',
      'setCountCannotNegative': 'تعداد دور نمی‌تواند منفی باشد',
      'validProgressNumber': 'یک عدد پیشرفت معتبر وارد کنید',
      'progressBetween': 'پیشرفت باید بین 0 و {max} باشد',
      'maxMinusCount': 'شمارش باقی‌مانده',
      'tap': 'ضربه',
      'groups': 'گروه‌ها',
      'newGroup': 'گروه جدید',
      'editGroup': 'ویرایش گروه',
      'groupName': 'نام گروه',
      'groupMembers': 'اعضا',
      'noBeadsInGroup': 'هنوز تسبیحی در این گروه نیست.',
      'addBead': 'افزودن تسبیح',
      'addBeads': 'افزودن تسبیح‌ها',
      'newBead': 'تسبیح جدید',
      'deleteGroup': 'حذف گروه',
      'deleteGroupConfirm': 'این گروه حذف شود؟ تسبیح‌ها حفظ می‌شوند.',
      'removeFromGroup': 'حذف از گروه',
      'noNotesAdded': 'یادداشتی اضافه نشده است.',
      'select': 'انتخاب',
      'selectAll': 'انتخاب همه',
      'selectedCount': '{count} انتخاب شده',
      'deleteSelectedConfirm': '{count} مورد انتخاب‌شده حذف شود؟',
      'deletedSelected': '{count} حذف شد',
    },
    'fr': {
      'appTitle': 'Compteur de Perles',
      'milestones': 'Perles',
      'switchToLight': 'Passer en mode clair',
      'switchToDark': 'Passer en mode sombre',
      'noMilestones': 'Aucune perle. Appuyez sur + pour en ajouter.',
      'statsTitle': 'Historique',
      'statsToday': 'Aujourd\'hui',
      'statsLast7Days': '7 jours',
      'statsTotal': 'Total',
      'deleted': 'supprimé',
      'undo': 'Annuler',
      'edit': 'Modifier',
      'duplicate': 'Dupliquer',
      'clear': 'Effacer',
      'delete': 'Supprimer',
      'count': 'Compte',
      'check': 'Seuil',
      'set': 'Série',
      'progress': 'Progression',
      'createMilestone': 'Créer des perles',
      'editMilestone': 'Modifier les perles',
      'title': 'Titre',
      'notes': 'Notes',
      'notesHint': 'Ajoutez des notes pour ces perles...',
      'countField': 'Compte',
      'checkInterval': 'Intervalle de seuil',
      'checkHelper': 'Doit être inférieur ou égal à la moitié du compte. Utilisez 0 pour aucun seuil.',
      'setCount': 'Compte de série',
      'setCountHelper':
          'Le compte de série doit être inférieur ou égal au compte.',
      'setCountReadonlyHelper':
          'Le compte de série ne peut être modifié que depuis l’écran de progression.',
      'vibrationIntensity': 'Intensité de vibration',
      'reminderTitle': 'Rappel',
      'reminderEnable': 'Activer le rappel',
      'reminderRepeatOnce': 'Une fois',
      'reminderRepeatDaily': 'Quotidien',
      'reminderRepeatWeekly': 'Hebdomadaire',
      'reminderRepeatMonthly': 'Mensuel',
      'reminderRepeatYearly': 'Annuel',
      'reminderRepeatCountLabel': 'Nombre de répétitions',
      'reminderRepeatCountHelper':
          'Nombre de fois où le rappel se déclenche avant de s\'arrêter (désactivé = répétition illimitée)',
      'reminderRepeatCountRangeError': 'Entrez un nombre entre 2 et 100',
      'reminderRepeatDaysLabel': 'Répéter le',
      'reminderDayOfMonthLabel': 'Jour du mois',
      'reminderYearlyMonthLabel': 'Mois',
      'reminderYearlyDayLabel': 'Jour',
      'reminderRecurrenceLabel': 'Répétition',
      'reminderMonthlyBasisLabel': 'Base mensuelle',
      'reminderYearlyBasisLabel': 'Base annuelle',
      'reminderBasisGregorian': 'Grégorien',
      'reminderBasisHijri': 'Hégire',
      'reminderPickDateTime': "Choisir la date et l'heure",
      'reminderPickDate': 'Choisir la date',
      'reminderPickTime': "Choisir l'heure",
      'reminderNotSet': 'Non défini',
      'reminderAnchorTime': 'Heure',
      'reminderAnchorPrayer': 'Heure de prière',
      'reminderOffsetOnTime': "À l'heure",
      'reminderOffsetBefore': 'Avant',
      'reminderOffsetAfter': 'Après',
      'reminderMinutesLabel': 'Minutes',
      'reminderSelectPrayer': 'Choisir la prière',
      'save': 'Enregistrer',
      'update': 'Mettre à jour',
      'requiredSuffix': 'est requis',
      'mustBeInteger': 'doit être un entier',
      'countPositive': 'Le compte doit être un entier positif',
      'checkGreaterThanZero': 'Le seuil doit être supérieur à 0',
      'checkHalfError': 'Le seuil ne peut pas dépasser la moitié du compte',
      'enterValidCountFirst': 'Saisissez d’abord un compte valide',
      'setCountNegative': 'Le compte de série ne peut pas être négatif',
      'setCountGreaterCount':
          'Le compte de série ne peut pas dépasser le compte',
      'setCountValueRequired': 'Le compte de série est requis',
      'itemNotFound': 'Élément introuvable',
      'resetProgressTitle': 'Réinitialiser la progression ?',
      'resetProgressBody': 'Cela remettra la progression actuelle à 0.',
      'cancel': 'Annuler',
      'reset': 'Réinitialiser',
      'editProgressAndSetCount':
          'Modifier la progression et le compte de série',
      'progressCount': 'Compte de progression',
      'setCountCannotNegative': 'Le compte de série ne peut pas être négatif',
      'validProgressNumber': 'Saisissez un nombre de progression valide',
      'progressBetween': 'La progression doit être entre 0 et {max}',
      'maxMinusCount': 'Compte restant',
      'tap': 'TAP',
      'groups': 'Groupes',
      'newGroup': 'Nouveau groupe',
      'editGroup': 'Modifier le groupe',
      'groupName': 'Nom du groupe',
      'groupMembers': 'Membres',
      'noBeadsInGroup': 'Aucune perle dans ce groupe pour l\u2019instant.',
      'addBead': 'Ajouter une perle',
      'addBeads': 'Ajouter des perles',
      'newBead': 'Nouvelle perle',
      'deleteGroup': 'Supprimer le groupe',
      'deleteGroupConfirm':
          'Supprimer ce groupe\u00A0? Ses perles sont conserv\u00E9es.',
      'removeFromGroup': 'Retirer du groupe',
      'noNotesAdded': 'Aucune note ajoutée.',
      'select': 'Sélectionner',
      'selectAll': 'Tout sélectionner',
      'selectedCount': '{count} sélectionnés',
      'deleteSelectedConfirm': 'Supprimer {count} éléments sélectionnés ?',
      'deletedSelected': '{count} supprimés',
    },
    'de': {
      'appTitle': 'Perlen-Zähler',
      'milestones': 'Perlen',
      'switchToLight': 'Zu hellem Modus wechseln',
      'switchToDark': 'Zu dunklem Modus wechseln',
      'noMilestones': 'Noch keine Perlen. Mit + hinzufügen.',
      'statsTitle': 'Verlauf',
      'statsToday': 'Heute',
      'statsLast7Days': '7 Tage',
      'statsTotal': 'Gesamt',
      'deleted': 'gelöscht',
      'undo': 'Rückgängig',
      'edit': 'Bearbeiten',
      'duplicate': 'Duplizieren',
      'clear': 'Löschen',
      'delete': 'Löschen',
      'count': 'Zähler',
      'check': 'Intervall',
      'set': 'Satz',
      'progress': 'Fortschritt',
      'createMilestone': 'Perlen erstellen',
      'editMilestone': 'Perlen bearbeiten',
      'title': 'Titel',
      'notes': 'Notizen',
      'notesHint': 'Notizen für diese Perlen hinzufügen...',
      'countField': 'Zähler',
      'checkInterval': 'Intervallprüfung',
      'checkHelper': 'Muss kleiner oder gleich der Hälfte des Zählers sein. 0 bedeutet keine Prüfpunkte.',
      'setCount': 'Satz-Zähler',
      'setCountHelper': 'Der Satz-Zähler muss kleiner/gleich dem Zähler sein.',
      'setCountReadonlyHelper':
          'Der Satz-Zähler kann nur im Fortschrittsbildschirm geändert werden.',
      'vibrationIntensity': 'Vibrationsintensität',
      'reminderTitle': 'Erinnerung',
      'reminderEnable': 'Erinnerung aktivieren',
      'reminderRepeatOnce': 'Einmalig',
      'reminderRepeatDaily': 'Täglich',
      'reminderRepeatWeekly': 'Wöchentlich',
      'reminderRepeatMonthly': 'Monatlich',
      'reminderRepeatYearly': 'Jährlich',
      'reminderRepeatCountLabel': 'Wiederholungsanzahl',
      'reminderRepeatCountHelper':
          'Wie oft der Reminder ausgelöst wird, bevor er stoppt (aus = unbegrenzt)',
      'reminderRepeatCountRangeError': 'Geben Sie eine Zahl von 2 bis 100 ein',
      'reminderRepeatDaysLabel': 'Wiederholen am',
      'reminderDayOfMonthLabel': 'Tag des Monats',
      'reminderYearlyMonthLabel': 'Monat',
      'reminderYearlyDayLabel': 'Tag',
      'reminderRecurrenceLabel': 'Wiederholung',
      'reminderMonthlyBasisLabel': 'Monatliche Basis',
      'reminderYearlyBasisLabel': 'Jährliche Basis',
      'reminderBasisGregorian': 'Gregorianisch',
      'reminderBasisHijri': 'Hidschri',
      'reminderPickDateTime': 'Datum & Uhrzeit wählen',
      'reminderPickDate': 'Datum wählen',
      'reminderPickTime': 'Uhrzeit wählen',
      'reminderNotSet': 'Nicht festgelegt',
      'reminderAnchorTime': 'Uhrzeit',
      'reminderAnchorPrayer': 'Gebetszeit',
      'reminderOffsetOnTime': 'Pünktlich',
      'reminderOffsetBefore': 'Vorher',
      'reminderOffsetAfter': 'Nachher',
      'reminderMinutesLabel': 'Minuten',
      'reminderSelectPrayer': 'Gebet auswählen',
      'save': 'Speichern',
      'update': 'Aktualisieren',
      'requiredSuffix': 'ist erforderlich',
      'mustBeInteger': 'muss eine Ganzzahl sein',
      'countPositive': 'Der Zähler muss positiv sein',
      'checkGreaterThanZero': 'Intervall muss größer als 0 sein',
      'checkHalfError':
          'Intervall darf nicht größer als die Hälfte des Zählers sein',
      'enterValidCountFirst': 'Bitte zuerst einen gültigen Zähler eingeben',
      'setCountNegative': 'Satz-Zähler darf nicht negativ sein',
      'setCountGreaterCount':
          'Satz-Zähler darf nicht größer als der Zähler sein',
      'setCountValueRequired': 'Satz-Zähler ist erforderlich',
      'itemNotFound': 'Eintrag nicht gefunden',
      'resetProgressTitle': 'Fortschritt zurücksetzen?',
      'resetProgressBody': 'Dies setzt den aktuellen Fortschritt auf 0 zurück.',
      'cancel': 'Abbrechen',
      'reset': 'Zurücksetzen',
      'editProgressAndSetCount': 'Fortschritt und Satz-Zähler bearbeiten',
      'progressCount': 'Fortschrittszähler',
      'setCountCannotNegative': 'Satz-Zähler darf nicht negativ sein',
      'validProgressNumber': 'Bitte eine gültige Fortschrittszahl eingeben',
      'progressBetween': 'Fortschritt muss zwischen 0 und {max} liegen',
      'maxMinusCount': 'Verbleibende Anzahl',
      'tap': 'TAP',
      'groups': 'Gruppen',
      'newGroup': 'Neue Gruppe',
      'editGroup': 'Gruppe bearbeiten',
      'groupName': 'Gruppenname',
      'groupMembers': 'Mitglieder',
      'noBeadsInGroup': 'Noch keine Perlen in dieser Gruppe.',
      'addBead': 'Perle hinzufügen',
      'addBeads': 'Perlen hinzufügen',
      'newBead': 'Neue Perle',
      'deleteGroup': 'Gruppe löschen',
      'deleteGroupConfirm': 'Diese Gruppe löschen? Die Perlen bleiben erhalten.',
      'removeFromGroup': 'Aus Gruppe entfernen',
      'noNotesAdded': 'Keine Notizen hinzugefügt.',
      'select': 'Auswählen',
      'selectAll': 'Alle auswählen',
      'selectedCount': '{count} ausgewählt',
      'deleteSelectedConfirm': '{count} ausgewählte Elemente löschen?',
      'deletedSelected': '{count} gelöscht',
    },
    'ar': {
      'appTitle': 'عداد المسبحة',
      'milestones': 'مِسْبَحَة',
      'switchToLight': 'التبديل إلى الوضع الفاتح',
      'switchToDark': 'التبديل إلى الوضع الداكن',
      'noMilestones': 'لا توجد مسبحة بعد. اضغط + للإضافة.',
      'statsTitle': 'السجل',
      'statsToday': 'اليوم',
      'statsLast7Days': '٧ أيام',
      'statsTotal': 'الإجمالي',
      'deleted': 'تم الحذف',
      'undo': 'تراجع',
      'edit': 'تعديل',
      'duplicate': 'نسخ',
      'clear': 'مسح',
      'delete': 'حذف',
      'count': 'العدد',
      'check': 'الفاصل',
      'set': 'الدفعة',
      'progress': 'التقدم',
      'createMilestone': 'إنشاء مسبحة',
      'editMilestone': 'تعديل المسبحة',
      'title': 'العنوان',
      'notes': 'ملاحظات',
      'notesHint': 'أضف ملاحظات لهذه المسبحة...',
      'countField': 'العدد',
      'checkInterval': 'فاصل التحقق',
      'checkHelper': 'يجب أن يكون أقل من أو يساوي نصف العدد. استخدم 0 لتعطيل نقاط التحقق.',
      'setCount': 'عدد الدفعات',
      'setCountHelper': 'يجب أن يكون عدد الدفعات أقل من أو يساوي العدد.',
      'setCountReadonlyHelper': 'يمكن تغيير عدد الدفعات فقط من شاشة التقدم.',
      'vibrationIntensity': 'شدة الاهتزاز',
      'reminderTitle': 'تذكير',
      'reminderEnable': 'تفعيل التذكير',
      'reminderRepeatOnce': 'مرة واحدة',
      'reminderRepeatDaily': 'يوميًا',
      'reminderRepeatWeekly': 'أسبوعي',
      'reminderRepeatMonthly': 'شهري',
      'reminderRepeatYearly': 'سنوي',
      'reminderRepeatCountLabel': 'عدد التكرار',
      'reminderRepeatCountHelper':
          'عدد مرات تشغيل التذكير قبل التوقف (إيقاف = يتكرر دائمًا)',
      'reminderRepeatCountRangeError': 'أدخل رقمًا من 2 إلى 100',
      'reminderRepeatDaysLabel': 'التكرار في',
      'reminderDayOfMonthLabel': 'يوم من الشهر',
      'reminderYearlyMonthLabel': 'الشهر',
      'reminderYearlyDayLabel': 'اليوم',
      'reminderRecurrenceLabel': 'التكرار',
      'reminderMonthlyBasisLabel': 'الأساس الشهري',
      'reminderYearlyBasisLabel': 'الأساس السنوي',
      'reminderBasisGregorian': 'ميلادي',
      'reminderBasisHijri': 'هجري',
      'reminderPickDateTime': 'اختر التاريخ والوقت',
      'reminderPickDate': 'اختر التاريخ',
      'reminderPickTime': 'اختر الوقت',
      'reminderNotSet': 'غير محدد',
      'reminderAnchorTime': 'الوقت',
      'reminderAnchorPrayer': 'وقت الصلاة',
      'reminderOffsetOnTime': 'في الوقت',
      'reminderOffsetBefore': 'قبل',
      'reminderOffsetAfter': 'بعد',
      'reminderMinutesLabel': 'الدقائق',
      'reminderSelectPrayer': 'اختر الصلاة',
      'save': 'حفظ',
      'update': 'تحديث',
      'requiredSuffix': 'مطلوب',
      'mustBeInteger': 'يجب أن يكون رقمًا صحيحًا',
      'countPositive': 'يجب أن يكون العدد رقمًا صحيحًا موجبًا',
      'checkGreaterThanZero': 'يجب أن يكون الفاصل أكبر من 0',
      'checkHalfError': 'لا يمكن أن يكون الفاصل أكبر من نصف العدد',
      'enterValidCountFirst': 'أدخل عددًا صالحًا أولاً',
      'setCountNegative': 'لا يمكن أن يكون عدد الدفعات سالبًا',
      'setCountGreaterCount': 'لا يمكن أن يتجاوز عدد الدفعات العدد',
      'setCountValueRequired': 'عدد الدفعات مطلوب',
      'itemNotFound': 'العنصر غير موجود',
      'resetProgressTitle': 'إعادة تعيين التقدم؟',
      'resetProgressBody': 'سيتم إعادة التقدم الحالي إلى 0.',
      'cancel': 'إلغاء',
      'reset': 'إعادة تعيين',
      'editProgressAndSetCount': 'تعديل التقدم وعدد الدفعات',
      'progressCount': 'عدد التقدم',
      'setCountCannotNegative': 'لا يمكن أن يكون عدد الدفعات سالبًا',
      'validProgressNumber': 'أدخل رقم تقدم صالحًا',
      'progressBetween': 'يجب أن يكون التقدم بين 0 و {max}',
      'maxMinusCount': 'العدد المتبقي',
      'tap': 'اضغط',
      'groups': 'المجموعات',
      'newGroup': 'مجموعة جديدة',
      'editGroup': 'تعديل المجموعة',
      'groupName': 'اسم المجموعة',
      'groupMembers': 'الأعضاء',
      'noBeadsInGroup': 'لا توجد مسبحات في هذه المجموعة بعد.',
      'addBead': 'إضافة مسبحة',
      'addBeads': 'إضافة مسبحات',
      'newBead': 'مسبحة جديدة',
      'deleteGroup': 'حذف المجموعة',
      'deleteGroupConfirm': 'حذف هذه المجموعة؟ ستبقى المسبحات.',
      'removeFromGroup': 'إزالة من المجموعة',
      'noNotesAdded': 'لم يتم إضافة ملاحظات.',
      'select': 'تحديد',
      'selectAll': 'تحديد الكل',
      'selectedCount': 'تم تحديد {count}',
      'deleteSelectedConfirm': 'حذف {count} من العناصر المحددة؟',
      'deletedSelected': 'تم حذف {count}',
    },
    'ja': {
      'appTitle': 'ビーズカウンター',
      'milestones': 'ビーズ',
      'switchToLight': 'ライトモードに切り替え',
      'switchToDark': 'ダークモードに切り替え',
      'language': '言語',
      'chooseLanguage': '言語を選択',
      'noMilestones': 'ビーズがありません。+ をタップして追加してください。',
      'statsTitle': '履歴',
      'statsToday': '今日',
      'statsLast7Days': '7日間',
      'statsTotal': '合計',
      'deleted': 'を削除しました',
      'undo': '元に戻す',
      'edit': '編集',
      'duplicate': '複製',
      'clear': 'クリア',
      'delete': '削除',
      'count': 'カウント',
      'check': 'チェック',
      'set': 'セット',
      'progress': '進捗',
      'createMilestone': 'ビーズを作成',
      'editMilestone': 'ビーズを編集',
      'title': 'タイトル',
      'notes': 'メモ',
      'notesHint': 'このビーズのメモを入力...',
      'countField': 'カウント',
      'checkInterval': 'チェック間隔',
      'checkHelper': 'カウントの半分以下である必要があります。0でチェックポイントなし。',
      'setCount': 'セット数',
      'setCountHelper': 'セット数はカウント以下である必要があります。',
      'setCountReadonlyHelper': 'セット数は進捗画面でのみ変更できます。',
      'vibrationIntensity': 'バイブ強度',
      'reminderTitle': 'リマインダー',
      'reminderEnable': 'リマインダーを有効にする',
      'reminderRepeatOnce': '一回',
      'reminderRepeatDaily': '毎日',
      'reminderRepeatWeekly': '毎週',
      'reminderRepeatMonthly': '毎月',
      'reminderRepeatYearly': '毎年',
      'reminderRepeatCountLabel': '繰り返し回数',
      'reminderRepeatCountHelper':
          'リマインダーが停止するまでの発火回数（オフ = 永久に繰り返す）',
      'reminderRepeatCountRangeError': '2〜100の数字を入力してください',
      'reminderRepeatDaysLabel': '繰り返す曜日',
      'reminderDayOfMonthLabel': '月の日',
      'reminderYearlyMonthLabel': '月',
      'reminderYearlyDayLabel': '日',
      'reminderRecurrenceLabel': '繰り返し',
      'reminderMonthlyBasisLabel': '毎月の基準',
      'reminderYearlyBasisLabel': '毎年の基準',
      'reminderBasisGregorian': 'グレゴリオ暦',
      'reminderBasisHijri': 'ヒジュラ暦',
      'reminderPickDateTime': '日時を選択',
      'reminderPickDate': '日付を選択',
      'reminderPickTime': '時刻を選択',
      'reminderNotSet': '未設定',
      'reminderAnchorTime': '時刻',
      'reminderAnchorPrayer': '礼拝時間',
      'reminderOffsetOnTime': '時刻通り',
      'reminderOffsetBefore': '前',
      'reminderOffsetAfter': '後',
      'reminderMinutesLabel': '分',
      'reminderSelectPrayer': '礼拝を選択',
      'save': '保存',
      'update': '更新',
      'requiredSuffix': 'は必須です',
      'mustBeInteger': 'は整数である必要があります',
      'countPositive': 'カウントは正の整数である必要があります',
      'checkGreaterThanZero': 'チェックは0より大きい必要があります',
      'checkHalfError': 'チェックはカウントの半分を超えられません',
      'enterValidCountFirst': '先に有効なカウントを入力してください',
      'setCountNegative': 'セット数は負にできません',
      'setCountGreaterCount': 'セット数はカウントを超えられません',
      'setCountValueRequired': 'セット数は必須です',
      'itemNotFound': '項目が見つかりません',
      'resetProgressTitle': '進捗をリセットしますか？',
      'resetProgressBody': '現在の進捗を0に戻します。',
      'cancel': 'キャンセル',
      'reset': 'リセット',
      'editProgressAndSetCount': '進捗とセット数を編集',
      'progressCount': '進捗カウント',
      'setCountCannotNegative': 'セット数は負にできません',
      'validProgressNumber': '有効な進捗値を入力してください',
      'progressBetween': '進捗は0から{max}の間である必要があります',
      'maxMinusCount': '残りカウント',
      'tap': 'タップ',
      'groups': 'グループ',
      'newGroup': '新しいグループ',
      'editGroup': 'グループを編集',
      'groupName': 'グループ名',
      'groupMembers': 'メンバー',
      'noBeadsInGroup': 'このグループにはまだ数珠がありません。',
      'addBead': '数珠を追加',
      'addBeads': '数珠を追加',
      'newBead': '新しい数珠',
      'deleteGroup': 'グループを削除',
      'deleteGroupConfirm': 'このグループを削除しますか？数珠は保持されます。',
      'removeFromGroup': 'グループから削除',
      'noNotesAdded': 'メモはありません。',
      'select': '選択',
      'selectAll': 'すべて選択',
      'selectedCount': '{count} 件選択中',
      'deleteSelectedConfirm': '選択した {count} 件を削除しますか？',
      'deletedSelected': '{count} 件を削除しました',
    },
    'zh': {
      'appTitle': '念珠计数器',
      'milestones': '念珠',
      'switchToLight': '切换到浅色模式',
      'switchToDark': '切换到深色模式',
      'noMilestones': '还没有念珠。点击 + 添加。',
      'statsTitle': '历史',
      'statsToday': '今天',
      'statsLast7Days': '7天',
      'statsTotal': '总计',
      'deleted': '已删除',
      'undo': '撤销',
      'edit': '编辑',
      'duplicate': '复制',
      'clear': '清除',
      'delete': '删除',
      'count': '计数',
      'check': '检查点',
      'set': '组数',
      'progress': '进度',
      'createMilestone': '创建念珠',
      'editMilestone': '编辑念珠',
      'title': '标题',
      'notes': '备注',
      'notesHint': '为此念珠添加备注...',
      'countField': '计数',
      'checkInterval': '检查间隔',
      'checkHelper': '必须小于或等于计数的一半。使用 0 表示无检查点。',
      'setCount': '组数计数',
      'setCountHelper': '组数计数必须小于或等于计数。',
      'setCountReadonlyHelper': '组数只能在进度页面修改。',
      'vibrationIntensity': '震动强度',
      'reminderTitle': '提醒',
      'reminderEnable': '启用提醒',
      'reminderRepeatOnce': '一次',
      'reminderRepeatDaily': '每天',
      'reminderRepeatWeekly': '每周',
      'reminderRepeatMonthly': '每月',
      'reminderRepeatYearly': '每年',
      'reminderRepeatCountLabel': '重复次数',
      'reminderRepeatCountHelper':
          '提醒停止前触发的次数（关闭 = 永远重复）',
      'reminderRepeatCountRangeError': '请输入 2 到 100 之间的数字',
      'reminderRepeatDaysLabel': '重复于',
      'reminderDayOfMonthLabel': '每月几号',
      'reminderYearlyMonthLabel': '月份',
      'reminderYearlyDayLabel': '日',
      'reminderRecurrenceLabel': '重复',
      'reminderMonthlyBasisLabel': '每月基准',
      'reminderYearlyBasisLabel': '每年基准',
      'reminderBasisGregorian': '公历',
      'reminderBasisHijri': '回历',
      'reminderPickDateTime': '选择日期和时间',
      'reminderPickDate': '选择日期',
      'reminderPickTime': '选择时间',
      'reminderNotSet': '未设置',
      'reminderAnchorTime': '时间',
      'reminderAnchorPrayer': '礼拜时间',
      'reminderOffsetOnTime': '准时',
      'reminderOffsetBefore': '之前',
      'reminderOffsetAfter': '之后',
      'reminderMinutesLabel': '分钟',
      'reminderSelectPrayer': '选择礼拜',
      'save': '保存',
      'update': '更新',
      'requiredSuffix': '为必填项',
      'mustBeInteger': '必须是整数',
      'countPositive': '计数必须是正整数',
      'checkGreaterThanZero': '检查点必须大于 0',
      'checkHalfError': '检查点不能大于计数的一半',
      'enterValidCountFirst': '请先输入有效计数',
      'setCountNegative': '组数不能为负数',
      'setCountGreaterCount': '组数不能大于计数',
      'setCountValueRequired': '组数为必填项',
      'itemNotFound': '未找到项目',
      'resetProgressTitle': '重置进度？',
      'resetProgressBody': '这会将当前进度重置为 0。',
      'cancel': '取消',
      'reset': '重置',
      'editProgressAndSetCount': '编辑进度与组数',
      'progressCount': '进度计数',
      'setCountCannotNegative': '组数不能为负数',
      'validProgressNumber': '请输入有效进度数字',
      'progressBetween': '进度必须在 0 到 {max} 之间',
      'maxMinusCount': '剩余计数',
      'tap': '点按',
      'groups': '分组',
      'newGroup': '新建分组',
      'editGroup': '编辑分组',
      'groupName': '分组名称',
      'groupMembers': '成员',
      'noBeadsInGroup': '该分组还没有念珠。',
      'addBead': '添加念珠',
      'addBeads': '添加念珠',
      'newBead': '新建念珠',
      'deleteGroup': '删除分组',
      'deleteGroupConfirm': '删除此分组？念珠将保留。',
      'removeFromGroup': '从分组移除',
      'noNotesAdded': '暂无备注。',
      'select': '选择',
      'selectAll': '全选',
      'selectedCount': '已选择 {count} 项',
      'deleteSelectedConfirm': '删除选中的 {count} 项？',
      'deletedSelected': '已删除 {count} 项',
    },
    'tr': {
      'appTitle': 'Tesbih Sayacı',
      'milestones': 'Tesbih',
      'switchToLight': 'Açık moda geç',
      'switchToDark': 'Koyu moda geç',
      'noMilestones': 'Henüz tesbih yok. Eklemek için + dokunun.',
      'statsTitle': 'Geçmiş',
      'statsToday': 'Bugün',
      'statsLast7Days': '7 gün',
      'statsTotal': 'Toplam',
      'deleted': 'silindi',
      'undo': 'Geri Al',
      'edit': 'Düzenle',
      'duplicate': 'Kopyala',
      'clear': 'Temizle',
      'delete': 'Sil',
      'count': 'Sayı',
      'check': 'Kontrol',
      'set': 'Set',
      'progress': 'İlerleme',
      'createMilestone': 'Tesbih Oluştur',
      'editMilestone': 'Tesbihi Düzenle',
      'title': 'Başlık',
      'notes': 'Notlar',
      'notesHint': 'Bu tesbih için not ekleyin...',
      'countField': 'Sayı',
      'checkInterval': 'Kontrol Aralığı',
      'checkHelper': 'Sayı değerinin yarısından küçük/eşit olmalı. Kontrol noktası olmaması için 0 girin.',
      'setCount': 'Set Sayısı',
      'setCountHelper': 'Set sayısı sayıdan büyük olamaz.',
      'setCountReadonlyHelper':
          'Set sayısı sadece ilerleme ekranından değiştirilebilir.',
      'vibrationIntensity': 'Titreşim Şiddeti',
      'reminderTitle': 'Hatırlatıcı',
      'reminderEnable': 'Hatırlatıcıyı etkinleştir',
      'reminderRepeatOnce': 'Bir kez',
      'reminderRepeatDaily': 'Her gün',
      'reminderRepeatWeekly': 'Haftalık',
      'reminderRepeatMonthly': 'Aylık',
      'reminderRepeatYearly': 'Yıllık',
      'reminderRepeatCountLabel': 'Tekrar sayısı',
      'reminderRepeatCountHelper':
          'Hatırlatmanın durmadan önce kaç kez çalacağı (kapalı = her zaman tekrarlanır)',
      'reminderRepeatCountRangeError': '2 ile 100 arasında bir sayı girin',
      'reminderRepeatDaysLabel': 'Tekrarla',
      'reminderDayOfMonthLabel': 'Ayın günü',
      'reminderYearlyMonthLabel': 'Ay',
      'reminderYearlyDayLabel': 'Gün',
      'reminderRecurrenceLabel': 'Tekrarlama',
      'reminderMonthlyBasisLabel': 'Aylık esas',
      'reminderYearlyBasisLabel': 'Yıllık esas',
      'reminderBasisGregorian': 'Miladi',
      'reminderBasisHijri': 'Hicri',
      'reminderPickDateTime': 'Tarih ve saat seç',
      'reminderPickDate': 'Tarih seç',
      'reminderPickTime': 'Saat seç',
      'reminderNotSet': 'Ayarlanmadı',
      'reminderAnchorTime': 'Saat',
      'reminderAnchorPrayer': 'Namaz vakti',
      'reminderOffsetOnTime': 'Vaktinde',
      'reminderOffsetBefore': 'Önce',
      'reminderOffsetAfter': 'Sonra',
      'reminderMinutesLabel': 'Dakika',
      'reminderSelectPrayer': 'Namaz seç',
      'save': 'Kaydet',
      'update': 'Güncelle',
      'requiredSuffix': 'zorunludur',
      'mustBeInteger': 'tam sayı olmalıdır',
      'countPositive': 'Sayı pozitif tam sayı olmalı',
      'checkGreaterThanZero': 'Kontrol 0’dan büyük olmalı',
      'checkHalfError': 'Kontrol sayı değerinin yarısından büyük olamaz',
      'enterValidCountFirst': 'Önce geçerli bir sayı girin',
      'setCountNegative': 'Set sayısı negatif olamaz',
      'setCountGreaterCount': 'Set sayısı sayıdan büyük olamaz',
      'setCountValueRequired': 'Set sayısı zorunludur',
      'itemNotFound': 'Öğe bulunamadı',
      'resetProgressTitle': 'İlerleme sıfırlansın mı?',
      'resetProgressBody': 'Bu işlem mevcut ilerlemeyi 0 yapar.',
      'cancel': 'İptal',
      'reset': 'Sıfırla',
      'editProgressAndSetCount': 'İlerleme ve set sayısını düzenle',
      'progressCount': 'İlerleme Sayısı',
      'setCountCannotNegative': 'Set sayısı negatif olamaz',
      'validProgressNumber': 'Geçerli bir ilerleme sayısı girin',
      'progressBetween': 'İlerleme 0 ile {max} arasında olmalı',
      'maxMinusCount': 'Kalan Sayı',
      'tap': 'DOKUN',
      'groups': 'Gruplar',
      'newGroup': 'Yeni Grup',
      'editGroup': 'Grubu Düzenle',
      'groupName': 'Grup adı',
      'groupMembers': 'Üyeler',
      'noBeadsInGroup': 'Bu grupta henüz tespih yok.',
      'addBead': 'Tespih ekle',
      'addBeads': 'Tespihler ekle',
      'newBead': 'Yeni Tespih',
      'deleteGroup': 'Grubu Sil',
      'deleteGroupConfirm': 'Bu grup silinsin mi? Tespihler kalır.',
      'removeFromGroup': 'Gruptan çıkar',
      'noNotesAdded': 'Not eklenmedi.',
      'select': 'Seç',
      'selectAll': 'Tümünü seç',
      'selectedCount': '{count} seçili',
      'deleteSelectedConfirm': '{count} seçili öğe silinsin mi?',
      'deletedSelected': '{count} silindi',
    },
    'ur': {
      'appTitle': 'تسبیح کاؤنٹر',
      'milestones': 'تسبیح',
      'switchToLight': 'لائٹ موڈ پر جائیں',
      'switchToDark': 'ڈارک موڈ پر جائیں',
      'noMilestones': 'ابھی کوئی تسبیح نہیں۔ شامل کرنے کے لیے + دبائیں۔',
      'statsTitle': 'تاریخ',
      'statsToday': 'آج',
      'statsLast7Days': '7 دن',
      'statsTotal': 'کل',
      'deleted': 'حذف ہوگیا',
      'undo': 'واپس لائیں',
      'edit': 'ترمیم',
      'duplicate': 'نقل کریں',
      'clear': 'صاف کریں',
      'delete': 'حذف',
      'count': 'گنتی',
      'check': 'چیک',
      'set': 'سیٹ',
      'progress': 'پیش رفت',
      'createMilestone': 'نئی تسبیح بنائیں',
      'editMilestone': 'تسبیح میں ترمیم',
      'title': 'عنوان',
      'notes': 'نوٹس',
      'notesHint': 'اس تسبیح کے لیے نوٹس لکھیں...',
      'countField': 'گنتی',
      'checkInterval': 'چیک وقفہ',
      'checkHelper': 'یہ گنتی کے نصف سے کم یا برابر ہونا چاہیے۔ بغیر چیک پوائنٹ کے لیے 0 درج کریں۔',
      'setCount': 'سیٹ گنتی',
      'setCountHelper': 'سیٹ گنتی، گنتی سے زیادہ نہیں ہونی چاہیے۔',
      'setCountReadonlyHelper':
          'سیٹ گنتی صرف پیش رفت اسکرین سے تبدیل ہو سکتی ہے۔',
      'vibrationIntensity': 'وائبریشن شدت',
      'reminderTitle': 'یاد دہانی',
      'reminderEnable': 'یاد دہانی فعال کریں',
      'reminderRepeatOnce': 'ایک بار',
      'reminderRepeatDaily': 'روزانہ',
      'reminderRepeatWeekly': 'ہفتہ وار',
      'reminderRepeatMonthly': 'ماہانہ',
      'reminderRepeatYearly': 'سالانہ',
      'reminderRepeatCountLabel': 'تکرار کی تعداد',
      'reminderRepeatCountHelper':
          'یاد دہانی رکنے سے پہلے کتنی بار چلے گی (بند = ہمیشہ دہرائی جائے)',
      'reminderRepeatCountRangeError': '2 سے 100 تک نمبر درج کریں',
      'reminderRepeatDaysLabel': 'دہرائیں',
      'reminderDayOfMonthLabel': 'مہینے کا دن',
      'reminderYearlyMonthLabel': 'مہینہ',
      'reminderYearlyDayLabel': 'دن',
      'reminderRecurrenceLabel': 'تکرار',
      'reminderMonthlyBasisLabel': 'ماہانہ بنیاد',
      'reminderYearlyBasisLabel': 'سالانہ بنیاد',
      'reminderBasisGregorian': 'عیسوی',
      'reminderBasisHijri': 'ہجری',
      'reminderPickDateTime': 'تاریخ اور وقت منتخب کریں',
      'reminderPickDate': 'تاریخ منتخب کریں',
      'reminderPickTime': 'وقت منتخب کریں',
      'reminderNotSet': 'سیٹ نہیں ہے',
      'reminderAnchorTime': 'وقت',
      'reminderAnchorPrayer': 'نماز کا وقت',
      'reminderOffsetOnTime': 'وقت پر',
      'reminderOffsetBefore': 'پہلے',
      'reminderOffsetAfter': 'بعد',
      'reminderMinutesLabel': 'منٹ',
      'reminderSelectPrayer': 'نماز منتخب کریں',
      'save': 'محفوظ کریں',
      'update': 'اپ ڈیٹ',
      'requiredSuffix': 'لازمی ہے',
      'mustBeInteger': 'عدد صحیح ہونا چاہیے',
      'countPositive': 'گنتی مثبت عدد صحیح ہونی چاہیے',
      'checkGreaterThanZero': 'چیک 0 سے بڑا ہونا چاہیے',
      'checkHalfError': 'چیک گنتی کے نصف سے زیادہ نہیں ہو سکتا',
      'enterValidCountFirst': 'پہلے درست گنتی درج کریں',
      'setCountNegative': 'سیٹ گنتی منفی نہیں ہو سکتی',
      'setCountGreaterCount': 'سیٹ گنتی، گنتی سے زیادہ نہیں ہو سکتی',
      'setCountValueRequired': 'سیٹ گنتی لازمی ہے',
      'itemNotFound': 'آئٹم نہیں ملا',
      'resetProgressTitle': 'پیش رفت ری سیٹ کریں؟',
      'resetProgressBody': 'اس سے موجودہ پیش رفت 0 ہو جائے گی۔',
      'cancel': 'منسوخ',
      'reset': 'ری سیٹ',
      'editProgressAndSetCount': 'پیش رفت اور سیٹ گنتی میں ترمیم',
      'progressCount': 'پیش رفت گنتی',
      'setCountCannotNegative': 'سیٹ گنتی منفی نہیں ہو سکتی',
      'validProgressNumber': 'درست پیش رفت نمبر درج کریں',
      'progressBetween': 'پیش رفت 0 اور {max} کے درمیان ہونی چاہیے',
      'maxMinusCount': 'باقی گنتی',
      'tap': 'ٹیپ',
      'groups': 'گروہ',
      'newGroup': 'نیا گروہ',
      'editGroup': 'گروہ میں ترمیم',
      'groupName': 'گروہ کا نام',
      'groupMembers': 'ارکان',
      'noBeadsInGroup': 'اس گروہ میں ابھی کوئی تسبیح نہیں۔',
      'addBead': 'تسبیح شامل کریں',
      'addBeads': 'تسبیحیں شامل کریں',
      'newBead': 'نئی تسبیح',
      'deleteGroup': 'گروہ حذف کریں',
      'deleteGroupConfirm': 'یہ گروہ حذف کریں؟ تسبیحیں محفوظ رہیں گی۔',
      'removeFromGroup': 'گروہ سے نکالیں',
      'noNotesAdded': 'کوئی نوٹس شامل نہیں کیے گئے۔',
      'select': 'منتخب کریں',
      'selectAll': 'سب منتخب کریں',
      'selectedCount': '{count} منتخب',
      'deleteSelectedConfirm': 'منتخب {count} اشیاء حذف کریں؟',
      'deletedSelected': '{count} حذف ہوگئے',
    },
    'ru': {
      'appTitle': 'Счётчик тасбиха',
      'milestones': 'Тасбих',
      'switchToLight': 'Переключить на светлую тему',
      'switchToDark': 'Переключить на тёмную тему',
      'language': 'Язык',
      'chooseLanguage': 'Выбрать язык',
      'noMilestones': 'Пока нет тасбиха. Нажмите +, чтобы добавить.',
      'statsTitle': 'История',
      'statsToday': 'Сегодня',
      'statsLast7Days': '7 дней',
      'statsTotal': 'Итого',
      'deleted': 'удалено',
      'undo': 'Отменить',
      'edit': 'Изменить',
      'duplicate': 'Дублировать',
      'clear': 'Очистить',
      'delete': 'Удалить',
      'count': 'Счёт',
      'check': 'Отметка',
      'set': 'Сет',
      'progress': 'Прогресс',
      'createMilestone': 'Создать тасбих',
      'editMilestone': 'Изменить тасбих',
      'title': 'Название',
      'notes': 'Заметки',
      'notesHint': 'Добавьте заметки для этого тасбиха...',
      'countField': 'Счёт',
      'checkInterval': 'Интервал отметки',
      'checkHelper': 'Должно быть меньше или равно половине счёта. 0 отключает контрольные точки.',
      'setCount': 'Счёт сета',
      'setCountHelper': 'Счёт сета должен быть меньше или равен счёту.',
      'setCountReadonlyHelper':
          'Счёт сета можно изменить только с экрана прогресса.',
      'vibrationIntensity': 'Интенсивность вибрации',
      'reminderTitle': 'Напоминание',
      'reminderEnable': 'Включить напоминание',
      'reminderRepeatOnce': 'Один раз',
      'reminderRepeatDaily': 'Ежедневно',
      'reminderRepeatWeekly': 'Еженедельно',
      'reminderRepeatMonthly': 'Ежемесячно',
      'reminderRepeatYearly': 'Ежегодно',
      'reminderRepeatCountLabel': 'Количество повторений',
      'reminderRepeatCountHelper':
          'Сколько раз напоминание сработает перед остановкой (выкл = повторять бесконечно)',
      'reminderRepeatCountRangeError': 'Введите число от 2 до 100',
      'reminderRepeatDaysLabel': 'Повторять в',
      'reminderDayOfMonthLabel': 'День месяца',
      'reminderYearlyMonthLabel': 'Месяц',
      'reminderYearlyDayLabel': 'День',
      'reminderRecurrenceLabel': 'Повтор',
      'reminderMonthlyBasisLabel': 'Основа месяца',
      'reminderYearlyBasisLabel': 'Основа года',
      'reminderBasisGregorian': 'Григорианский',
      'reminderBasisHijri': 'Хиджра',
      'reminderPickDateTime': 'Выбрать дату и время',
      'reminderPickDate': 'Выбрать дату',
      'reminderPickTime': 'Выбрать время',
      'reminderNotSet': 'Не задано',
      'reminderAnchorTime': 'Время',
      'reminderAnchorPrayer': 'Время намаза',
      'reminderOffsetOnTime': 'Вовремя',
      'reminderOffsetBefore': 'До',
      'reminderOffsetAfter': 'После',
      'reminderMinutesLabel': 'Минуты',
      'reminderSelectPrayer': 'Выбрать намаз',
      'save': 'Сохранить',
      'update': 'Обновить',
      'requiredSuffix': 'обязательно',
      'mustBeInteger': 'должно быть целым числом',
      'countPositive': 'Счёт должен быть положительным целым числом',
      'checkGreaterThanZero': 'Отметка должна быть больше 0',
      'checkHalfError': 'Отметка не может быть больше половины счёта',
      'enterValidCountFirst': 'Сначала введите корректный счёт',
      'setCountNegative': 'Счёт сета не может быть отрицательным',
      'setCountGreaterCount': 'Счёт сета не может быть больше счёта',
      'setCountValueRequired': 'Счёт сета обязателен',
      'itemNotFound': 'Элемент не найден',
      'resetProgressTitle': 'Сбросить прогресс?',
      'resetProgressBody': 'Это вернёт текущий прогресс к 0.',
      'cancel': 'Отмена',
      'reset': 'Сбросить',
      'editProgressAndSetCount': 'Изменить прогресс и счёт сета',
      'progressCount': 'Счёт прогресса',
      'setCountCannotNegative': 'Счёт сета не может быть отрицательным',
      'validProgressNumber': 'Введите корректный номер прогресса',
      'progressBetween': 'Прогресс должен быть между 0 и {max}',
      'maxMinusCount': 'Осталось',
      'tap': 'ТАП',
      'groups': 'Группы',
      'newGroup': 'Новая группа',
      'editGroup': 'Изменить группу',
      'groupName': 'Название группы',
      'groupMembers': 'Участники',
      'noBeadsInGroup': 'В этой группе пока нет чёток.',
      'addBead': 'Добавить чётки',
      'addBeads': 'Добавить чётки',
      'newBead': 'Новые чётки',
      'deleteGroup': 'Удалить группу',
      'deleteGroupConfirm': 'Удалить эту группу? Чётки сохранятся.',
      'removeFromGroup': 'Убрать из группы',
      'noNotesAdded': 'Заметки не добавлены.',
      'select': 'Выбрать',
      'selectAll': 'Выбрать все',
      'selectedCount': 'Выбрано: {count}',
      'deleteSelectedConfirm': 'Удалить выбранные элементы ({count})?',
      'deletedSelected': 'Удалено: {count}',
    },
    'bn': {
      'appTitle': 'তসবীহ গণক',
      'milestones': 'তসবীহ',
      'switchToLight': 'লাইট মোডে যান',
      'switchToDark': 'ডার্ক মোডে যান',
      'language': 'ভাষা',
      'chooseLanguage': 'ভাষা নির্বাচন করুন',
      'noMilestones': 'কোন তসবীহ নেই। যোগ করতে + চাপুন।',
      'statsTitle': 'ইতিহাস',
      'statsToday': 'আজ',
      'statsLast7Days': '৭ দিন',
      'statsTotal': 'মোট',
      'deleted': 'মুছে ফেলা হয়েছে',
      'undo': 'পূর্বাবস্থায় ফেরান',
      'edit': 'সম্পাদনা',
      'duplicate': 'অনুলিপি',
      'clear': 'সাফ করুন',
      'delete': 'মুছুন',
      'count': 'গণনা',
      'check': 'চেক',
      'set': 'সেট',
      'progress': 'অগ্রগতি',
      'createMilestone': 'তসবীহ তৈরি করুন',
      'editMilestone': 'তসবীহ সম্পাদনা করুন',
      'title': 'শিরোনাম',
      'notes': 'নোট',
      'notesHint': 'এই তসবীহের জন্য নোট যোগ করুন...',
      'countField': 'গণনা',
      'checkInterval': 'চেক ব্যবধান',
      'checkHelper': 'মোট গণনার অর্ধেকের সমান বা কম। চেকপয়েন্ট না চাইলে খালি রাখুন বা ০ দিন।',
      'setCount': 'সেট সংখ্যা',
      'setCountHelper': 'সেট সংখ্যা মোট গণনার সমান বা কম হতে হবে।',
      'setCountReadonlyHelper': 'সেট সংখ্যা কেবল অগ্রগতি স্ক্রিন থেকে পরিবর্তন করা যাবে।',
      'vibrationIntensity': 'কম্পনের মাত্রা',
      'reminderTitle': 'অনুস্মারক',
      'reminderEnable': 'অনুস্মারক চালু করুন',
      'reminderRepeatOnce': 'একবার',
      'reminderRepeatDaily': 'প্রতিদিন',
      'reminderRepeatWeekly': 'সাপ্তাহিক',
      'reminderRepeatMonthly': 'মাসিক',
      'reminderRepeatYearly': 'বার্ষিক',
      'reminderRepeatCountLabel': 'পুনরাবৃত্তির সংখ্যা',
      'reminderRepeatCountHelper': 'থামার আগে অনুস্মারকটি যতবার বাজবে (বন্ধ = চিরতরে পুনরাবৃত্তি)',
      'reminderRepeatCountRangeError': '২ থেকে ১০০ এর মধ্যে একটি সংখ্যা লিখুন',
      'reminderRepeatDaysLabel': 'পুনরাবৃত্তির দিন',
      'reminderDayOfMonthLabel': 'মাসের দিন',
      'reminderYearlyMonthLabel': 'মাস',
      'reminderYearlyDayLabel': 'দিন',
      'reminderRecurrenceLabel': 'পুনরাবৃত্তি',
      'reminderMonthlyBasisLabel': 'মাসিক ভিত্তি',
      'reminderYearlyBasisLabel': 'বার্ষিক ভিত্তি',
      'reminderBasisGregorian': 'গ্রেগরিয়ান',
      'reminderBasisHijri': 'হিজরি',
      'reminderPickDateTime': 'তারিখ ও সময় নির্বাচন করুন',
      'reminderPickDate': 'তারিখ নির্বাচন করুন',
      'reminderPickTime': 'সময় নির্বাচন করুন',
      'reminderNotSet': 'সেট করা হয়নি',
      'reminderAnchorTime': 'সময়',
      'reminderAnchorPrayer': 'নামাজের সময়',
      'reminderOffsetOnTime': 'সঠিক সময়ে',
      'reminderOffsetBefore': 'আগে',
      'reminderOffsetAfter': 'পরে',
      'reminderMinutesLabel': 'মিনিট',
      'reminderSelectPrayer': 'নামাজ নির্বাচন করুন',
      'save': 'সংরক্ষণ',
      'update': 'আপডেট',
      'requiredSuffix': 'প্রয়োজন',
      'mustBeInteger': 'একটি পূর্ণসংখ্যা হতে হবে',
      'countPositive': 'গণনা একটি ধনাত্মক পূর্ণসংখ্যা হতে হবে',
      'checkGreaterThanZero': 'চেক ০ এর বেশি হতে হবে',
      'checkHalfError': 'চেক গণনার অর্ধেকের বেশি হতে পারে না',
      'enterValidCountFirst': 'প্রথমে একটি বৈধ গণনা লিখুন',
      'setCountNegative': 'সেট সংখ্যা ঋণাত্মক হতে পারে না',
      'setCountGreaterCount': 'সেট সংখ্যা গণনার চেয়ে বেশি হতে পারে না',
      'setCountValueRequired': 'সেট সংখ্যা আবশ্যক',
      'itemNotFound': 'আইটেম পাওয়া যায়নি',
      'resetProgressTitle': 'অগ্রগতি রিসেট করবেন?',
      'resetProgressBody': 'এটি বর্তমান অগ্রগতি ০-তে ফিরিয়ে দেবে।',
      'cancel': 'বাতিল',
      'reset': 'রিসেট',
      'editProgressAndSetCount': 'অগ্রগতি ও সেট সংখ্যা সম্পাদনা করুন',
      'progressCount': 'অগ্রগতি গণনা',
      'setCountCannotNegative': 'সেট সংখ্যা ঋণাত্মক হতে পারে না',
      'validProgressNumber': 'একটি বৈধ অগ্রগতি সংখ্যা লিখুন',
      'progressBetween': 'অগ্রগতি ০ এবং {max}-এর মধ্যে হতে হবে',
      'maxMinusCount': 'অবশিষ্ট গণনা',
      'tap': 'ট্যাপ',
      'groups': 'গ্রুপসমূহ',
      'newGroup': 'নতুন গ্রুপ',
      'editGroup': 'গ্রুপ সম্পাদনা',
      'groupName': 'গ্রুপের নাম',
      'groupMembers': 'সদস্যগণ',
      'noBeadsInGroup': 'এই গ্রুপে এখনও কোনও তসবীহ নেই।',
      'addBead': 'তসবীহ যোগ করুন',
      'addBeads': 'তসবীহসমূহ যোগ করুন',
      'newBead': 'নতুন তসবীহ',
      'deleteGroup': 'গ্রুপ মুছুন',
      'deleteGroupConfirm': 'এই গ্রুপটি মুছবেন? এর তসবীহগুলি সংরক্ষিত থাকবে।',
      'removeFromGroup': 'গ্রুপ থেকে সরান',
      'noNotesAdded': 'কোন নোট যোগ করা হয়নি।',
      'select': 'নির্বাচন করুন',
      'selectAll': 'সব নির্বাচন করুন',
      'selectedCount': '{count}টি নির্বাচিত',
      'deleteSelectedConfirm': '{count}টি নির্বাচিত আইটেম মুছবেন?',
      'deletedSelected': '{count}টি মুছে ফেলা হয়েছে',
    },
    'ta': {
      'appTitle': 'தஸ்பீஹ் கவுண்டர்',
      'milestones': 'மணிகள்',
      'switchToLight': 'லைட் மோடுக்கு மாற்றுக',
      'switchToDark': 'டார்க் மோடுக்கு மாற்றுக',
      'language': 'மொழி',
      'chooseLanguage': 'மொழியைத் தேர்ந்தெடுக்கவும்',
      'noMilestones': 'மணிகள் எதுவும் இல்லை. சேர்க்க + ஐத் தட்டவும்.',
      'statsTitle': 'வரலாறு',
      'statsToday': 'இன்று',
      'statsLast7Days': '7 நாட்கள்',
      'statsTotal': 'மொத்தம்',
      'deleted': 'நீக்கப்பட்டது',
      'undo': 'செயல்தவிர்',
      'edit': 'திருத்து',
      'duplicate': 'நகல் எடு',
      'clear': 'அழி',
      'delete': 'நீக்கு',
      'count': 'எண்ணிக்கை',
      'check': 'சரிபார்',
      'set': 'அமை',
      'progress': 'முன்னேற்றம்',
      'createMilestone': 'மணி உருவாக்கு',
      'editMilestone': 'மணி திருத்து',
      'title': 'தலைப்பு',
      'notes': 'குறிப்புகள்',
      'notesHint': 'இந்த மணிகளுக்கான குறிப்புகளைச் சேர்க்கவும்...',
      'countField': 'எண்ணிக்கை',
      'checkInterval': 'சரிபார்ப்பு இடைவெளி',
      'checkHelper': 'எண்ணிக்கையின் பாதியை விட குறைவாகவோ அல்லது சமமாகவோ இருக்க வேண்டும். சோதனை புள்ளிகள் தேவையில்லை எனில் காலியாக விடவும் அல்லது 0 வைக்கவும்.',
      'setCount': 'செட் எண்ணிக்கை',
      'setCountHelper': 'செட் எண்ணிக்கை மொத்த எண்ணிக்கைக்கு சமமாகவோ குறைவாகவோ இருக்க வேண்டும்.',
      'setCountReadonlyHelper': 'செட் எண்ணிக்கையை முன்னேற்றத் திரையிலிருந்து மட்டுமே மாற்ற முடியும்.',
      'vibrationIntensity': 'அதிர்வு தீவிரம்',
      'reminderTitle': 'நினைவூட்டல்',
      'reminderEnable': 'நினைவூட்டலை இயக்கு',
      'reminderRepeatOnce': 'ஒரு முறை',
      'reminderRepeatDaily': 'தினசரி',
      'reminderRepeatWeekly': 'வாராந்திர',
      'reminderRepeatMonthly': 'மாதாந்திர',
      'reminderRepeatYearly': 'வருடாந்திர',
      'reminderRepeatCountLabel': 'மீண்டும் நிகழும் எண்ணிக்கை',
      'reminderRepeatCountHelper': 'நினைவூட்டல் நிறுத்தப்படுவதற்கு முன் ஒலிக்கும் முறைகளின் எண்ணிக்கை (ஆஃப் = எப்போதும் மீண்டும் நிகழும்)',
      'reminderRepeatCountRangeError': '2 முதல் 100 வரையிலான எண்ணை உள்ளிடவும்',
      'reminderRepeatDaysLabel': 'மீண்டும் நிகழும் நாட்கள்',
      'reminderDayOfMonthLabel': 'மாதத்தின் நாள்',
      'reminderYearlyMonthLabel': 'மாதம்',
      'reminderYearlyDayLabel': 'நாள்',
      'reminderRecurrenceLabel': 'மீண்டும் செய்',
      'reminderMonthlyBasisLabel': 'மாதாந்திர அடிப்படை',
      'reminderYearlyBasisLabel': 'வருடாந்திர அடிப்படை',
      'reminderBasisGregorian': 'கிரிகோரியன்',
      'reminderBasisHijri': 'ஹிஜ்ரி',
      'reminderPickDateTime': 'தேதி & நேரத்தைத் தேர்ந்தெடுக்கவும்',
      'reminderPickDate': 'தேதியைத் தேர்ந்தெடுக்கவும்',
      'reminderPickTime': 'நேரத்தைத் தேர்ந்தெடுக்கவும்',
      'reminderNotSet': 'அமைக்கப்படவில்லை',
      'reminderAnchorTime': 'நேரம்',
      'reminderAnchorPrayer': 'தொழுகை நேரம்',
      'reminderOffsetOnTime': 'சரியான நேரத்தில்',
      'reminderOffsetBefore': 'முன்பு',
      'reminderOffsetAfter': 'பின்பு',
      'reminderMinutesLabel': 'நிமிடங்கள்',
      'reminderSelectPrayer': 'தொழுகையைத் தேர்ந்தெடுக்கவும்',
      'save': 'சேமி',
      'update': 'புதுப்பி',
      'requiredSuffix': 'தேவைப்படுகிறது',
      'mustBeInteger': 'முழு எண்ணாக இருக்க வேண்டும்',
      'countPositive': 'எண்ணிக்கை ஒரு நேர்மறை முழு எண்ணாக இருக்க வேண்டும்',
      'checkGreaterThanZero': 'சரிபார்ப்பு 0-ஐ விட அதிகமாக இருக்க வேண்டும்',
      'checkHalfError': 'சரிபார்ப்பு எண்ணிக்கையின் பாதியை விட அதிகமாக இருக்க முடியாது',
      'enterValidCountFirst': 'முதலில் செல்லுபடியாகும் எண்ணிக்கையை உள்ளிடவும்',
      'setCountNegative': 'செட் எண்ணிக்கை எதிர்மறையாக இருக்க முடியாது',
      'setCountGreaterCount': 'செட் எண்ணிக்கை மொத்த எண்ணிக்கையை விட அதிகமாக இருக்க முடியாது',
      'setCountValueRequired': 'செட் எண்ணிக்கை தேவை',
      'itemNotFound': 'பொருள் கிடைக்கவில்லை',
      'resetProgressTitle': 'முன்னேற்றத்தை மீட்டமைக்கவா?',
      'resetProgressBody': 'இது தற்போதைய முன்னேற்றத்தை 0-க்கு மீட்டமைக்கும்.',
      'cancel': 'ரத்து',
      'reset': 'மீட்டமை',
      'editProgressAndSetCount': 'முன்னேற்றம் மற்றும் செட் எண்ணிக்கையைத் திருத்து',
      'progressCount': 'முன்னேற்ற எண்ணிக்கை',
      'setCountCannotNegative': 'செட் எண்ணிக்கை எதிர்மறையாக இருக்க முடியாது',
      'validProgressNumber': 'செல்லுபடியாகும் முன்னேற்ற எண்ணை உள்ளிடவும்',
      'progressBetween': 'முன்னேற்றம் 0 மற்றும் {max} இடையே இருக்க வேண்டும்',
      'maxMinusCount': 'மீதமுள்ள எண்ணிக்கை',
      'tap': 'தட்டு',
      'groups': 'குழுக்கள்',
      'newGroup': 'புதிய குழு',
      'editGroup': 'குழுவைத் திருத்து',
      'groupName': 'குழு பெயர்',
      'groupMembers': 'உறுப்பினர்கள்',
      'noBeadsInGroup': 'இந்த குழுவில் இதுவரை மணிகள் எதுவும் இல்லை.',
      'addBead': 'மணி சேர்',
      'addBeads': 'மணிகளைச் சேர்',
      'newBead': 'புதிய மணி',
      'deleteGroup': 'குழுவை நீக்கு',
      'deleteGroupConfirm': 'இந்த குழுவை நீக்கவா? அதன் மணிகள் பாதுகாக்கப்படும்.',
      'removeFromGroup': 'குழுவிலிருந்து அகற்று',
      'noNotesAdded': 'குறிப்புகள் எதுவும் சேர்க்கப்படவில்லை.',
      'select': 'தேர்ந்தெடு',
      'selectAll': 'அனைத்தையும் தேர்ந்தெடு',
      'selectedCount': '{count} தேர்ந்தெடுக்கப்பட்டது',
      'deleteSelectedConfirm': 'தேர்ந்தெடுக்கப்பட்ட {count} உருப்படிகளை நீக்கவா?',
      'deletedSelected': '{count} நீக்கப்பட்டது',
    },
  };

  String _t(String key) {
    final code = locale.languageCode;
    final langMap = _strings[code] ?? _strings['en']!;
    return langMap[key] ?? _strings['en']![key] ?? key;
  }

  String get appTitle => _t('appTitle');
  String get milestones => _t('milestones');
  String get switchToLight => _t('switchToLight');
  String get switchToDark => _t('switchToDark');
  String get language => _t('language');
  String get chooseLanguage => _t('chooseLanguage');
  String get noMilestones => _t('noMilestones');
  String get statsTitle => _t('statsTitle');
  String get statsToday => _t('statsToday');
  String get statsLast7Days => _t('statsLast7Days');
  String get statsTotal => _t('statsTotal');
  String deletedItem(String title) => '"$title" ${_t('deleted')}';
  String get undo => _t('undo');
  String get edit => _t('edit');
  String get delete => _t('delete');
  String get count => _t('count');
  String get check => _t('check');
  String get set => _t('set');
  String get progress => _t('progress');
  String get createMilestone => _t('createMilestone');
  String get editMilestone => _t('editMilestone');
  String get title => _t('title');
  String get notes => _t('notes');
  String get notesHint => _t('notesHint');
  String get countField => _t('countField');
  String get checkInterval => _t('checkInterval');
  String get checkHelper => _t('checkHelper');
  String get setCount => _t('setCount');
  String get setCountHelper => _t('setCountHelper');
  String get setCountReadonlyHelper => _t('setCountReadonlyHelper');
  String get vibrationIntensity => _t('vibrationIntensity');
  String get reminderTitle => _t('reminderTitle');
  String get reminderEnable => _t('reminderEnable');
  String get reminderRepeatOnce => _t('reminderRepeatOnce');
  String get reminderRepeatDaily => _t('reminderRepeatDaily');
  String get reminderRepeatWeekly => _t('reminderRepeatWeekly');
  String get reminderRepeatMonthly => _t('reminderRepeatMonthly');
  String get reminderRepeatYearly => _t('reminderRepeatYearly');
  String get reminderRepeatCountLabel => _t('reminderRepeatCountLabel');
  String get reminderRepeatCountHelper => _t('reminderRepeatCountHelper');
  String get reminderRepeatCountRangeError => _t('reminderRepeatCountRangeError');
  String get reminderRepeatDaysLabel => _t('reminderRepeatDaysLabel');
  String get reminderDayOfMonthLabel => _t('reminderDayOfMonthLabel');
  String get reminderYearlyMonthLabel => _t('reminderYearlyMonthLabel');
  String get reminderYearlyDayLabel => _t('reminderYearlyDayLabel');
  String get reminderRecurrenceLabel => _t('reminderRecurrenceLabel');
  String get reminderMonthlyBasisLabel => _t('reminderMonthlyBasisLabel');
  String get reminderYearlyBasisLabel => _t('reminderYearlyBasisLabel');
  String get reminderBasisGregorian => _t('reminderBasisGregorian');
  String get reminderBasisHijri => _t('reminderBasisHijri');
  String get reminderPickDateTime => _t('reminderPickDateTime');
  String get reminderPickDate => _t('reminderPickDate');
  String get reminderPickTime => _t('reminderPickTime');
  String get reminderNotSet => _t('reminderNotSet');
  String get reminderAnchorTime => _t('reminderAnchorTime');
  String get reminderAnchorPrayer => _t('reminderAnchorPrayer');
  String get reminderOffsetOnTime => _t('reminderOffsetOnTime');
  String get reminderOffsetBefore => _t('reminderOffsetBefore');
  String get reminderOffsetAfter => _t('reminderOffsetAfter');
  String get reminderMinutesLabel => _t('reminderMinutesLabel');
  String get reminderSelectPrayer => _t('reminderSelectPrayer');
  String get save => _t('save');
  String get update => _t('update');
  String requiredField(String fieldName) =>
      '$fieldName ${_t('requiredSuffix')}';
  String fieldMustBeInteger(String fieldName) =>
      '$fieldName ${_t('mustBeInteger')}';
  String get countPositive => _t('countPositive');
  String get checkGreaterThanZero => _t('checkGreaterThanZero');
  String get checkHalfError => _t('checkHalfError');
  String get enterValidCountFirst => _t('enterValidCountFirst');
  String get setCountNegative => _t('setCountNegative');
  String get setCountGreaterCount => _t('setCountGreaterCount');
  String get setCountValueRequired => _t('setCountValueRequired');
  String get itemNotFound => _t('itemNotFound');
  String get resetProgressTitle => _t('resetProgressTitle');
  String get resetProgressBody => _t('resetProgressBody');
  String get cancel => _t('cancel');
  String get reset => _t('reset');
  String get editProgressAndSetCount => _t('editProgressAndSetCount');
  String get progressCount => _t('progressCount');
  String get setCountCannotNegative => _t('setCountCannotNegative');
  String get validProgressNumber => _t('validProgressNumber');
  String progressBetween(int max) =>
      _t('progressBetween').replaceAll('{max}', '$max');
  String get maxMinusCount => _t('maxMinusCount');
  String get tap => _t('tap');
  String get noNotesAdded => _t('noNotesAdded');
  String get groups => _t('groups');
  String get newGroup => _t('newGroup');
  String get editGroup => _t('editGroup');
  String get groupName => _t('groupName');
  String get groupMembers => _t('groupMembers');
  String get noBeadsInGroup => _t('noBeadsInGroup');
  String get addBead => _t('addBead');
  String get addBeads => _t('addBeads');
  String get newBead => _t('newBead');
  String get deleteGroup => _t('deleteGroup');
  String get deleteGroupConfirm => _t('deleteGroupConfirm');
  String get removeFromGroup => _t('removeFromGroup');
  String get duplicate => _t('duplicate');
  String get clear => _t('clear');
  String get select => _t('select');
  String get selectAll => _t('selectAll');
  String selectedCount(int count) =>
      _t('selectedCount').replaceAll('{count}', '$count');
  String deleteSelectedConfirm(int count) =>
      _t('deleteSelectedConfirm').replaceAll('{count}', '$count');
  String deletedSelected(int count) =>
      _t('deletedSelected').replaceAll('{count}', '$count');
}

class _TesbihatLocalizationsDelegate
    extends LocalizationsDelegate<TesbihatLocalizations> {
  const _TesbihatLocalizationsDelegate();

  @override
  bool isSupported(Locale locale) => TesbihatLocalizations.supportedLocales.any(
    (l) => l.languageCode == locale.languageCode,
  );

  @override
  Future<TesbihatLocalizations> load(Locale locale) async {
    return TesbihatLocalizations(locale);
  }

  @override
  bool shouldReload(
    covariant LocalizationsDelegate<TesbihatLocalizations> old,
  ) {
    return false;
  }
}

extension TesbihatLocalizationsX on BuildContext {
  TesbihatLocalizations get tesbihatL10n => TesbihatLocalizations.of(this);
}
